...
while true; do
    select host_type in HTTP HTTPS QUITTER; do
        ...
        \n Redirect / https://$domain_name \
            ...
            HTTPS)
                cp $TEMPLATE  $CONFDIR/$domain_name.conf
                sed s/nom_de_domaine/$domain_name.local/ $CONFDIR/$domain_name.conf > $CONFDIR/$domain_name.conf
                sed s/racine_projet/$WEBDIR$project_path/g $CONFDIR/$domain_name.conf > $CONFDIR/$domain_name.conf
                sed s/core_folder/$core_folder/g $CONFDIR/$domain_name.conf > $CONFDIR/$domain_name.conf
                sed s/project_name/$project_name/g $CONFDIR/$domain_name.conf > $CONFDIR/$domain_name.conf
                sed s/env_path/$env_path/g $CONFDIR/$domain_name.conf > $CONFDIR/$domain_name.conf
                sudo chown :www-data $project_path
                #sed -i "s/racine_projet/$domain_name/g" /etc/apache2/sites-available/siepki.org.conf
                # sed -i "/ServerName/a  DocumentRoot $WEBDIR$domain_name" /etc/apache2/sites-available/$domain_name.conf
                echo "Nous venons de configurer le fichier, nous allons à
